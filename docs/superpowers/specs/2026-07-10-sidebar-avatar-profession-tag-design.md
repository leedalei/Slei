...
调整侧边栏私信列表中的成员行：将头像从当前 28px 调整到 30px，并在成员名称右侧显示职业标签；同时让全局头像统一使用 1px 中灰色实线边框，并修正私信列表右侧滚动条补偿间距。

## 范围

- 仅影响 `WorkspaceSidebar` 的私信列表行；私信行保持 40px 高度以容纳 30px 头像，频道行继续保持现有 32px 高度。
- 私信列表容器增加 `pr-2`，补偿右侧滚动条宽度，使左右外边距视觉一致；频道列表不增加该间距。
...
为 `MemberAvatar` 增加名为 `sidebar` 的专用 30px 尺寸变体，并仅在私信列表传入该尺寸。私信列表行保持 40px 高度，触发器继续占满行高；频道行不变。职业标签复用现有 `Badge` 组件和 secondary 视觉样式，放在名称之后；名称和标签放在 `min-w-0` 的单行 flex 容器内，标签设置最大宽度并允许截断，确保右侧菜单按钮仍可用。
...
- 为私信列表行增加 DOM 测试：验证私信列表容器的 `pr-2`、私信行的 40px 高度、触发器满高、`data-avatar-size="sidebar"`、`size-[1.875rem]`（30px）和职业标签文本。
