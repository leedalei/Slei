...
设置 overlay 覆盖整个应用内容区域，视觉上是一整页连续的设置内容，而不是外层里再放两个分离卡片或两个浮动面板。左侧导航和右侧详情属于同一个页面容器，左右连在一起；层次通过分割线、右侧左边界和轻量阴影表达。

- 左上角：返回按钮，文案为“返回应用”或同等本地化文案。
- 左栏顶部：设置搜索输入。
- 左栏主体：按分组展示设置项。
- 中间分隔：左侧导航和右侧详情之间使用连续的 `border-left` 或同等视觉分隔，不使用外部间距把两侧断开。
- 右侧详情：展示当前设置项对应内容，并独立滚动；右侧内容区左边需要使用 `border-left` 和 `shadow-left` 风格的左侧阴影来区分层次。

页面形态要求：

- overlay 背景和左右两栏应共同构成一个完整页面。
- 左侧导航宽度固定或有清晰响应式宽度，右侧详情占据剩余空间。
- 左侧和右侧之间不能出现明显 gutter、卡片外边距或浮动容器效果。
- 右侧详情不应被设计成独立卡片；详情内部可以保留表单行、列表项、确认弹窗和必要的重复 item card。
- 右侧左边界的阴影应轻量克制，只用于表达内容层级，不做装饰性渐变或大面积背景效果。
