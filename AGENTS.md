...
- 任务消息、任务卡片、coordinator 建任务和任务线程入口必须遵守 `docs/architecture/0006-task-source-message-card.md`；任务卡片是源消息的展示状态，不是新增消息。若行为、DTO、UX 或兼容策略变化，必须在同一任务中同步更新该 ADR，避免功能漂移。
...
- 涉及消息会话、消息气泡、消息 header、消息操作区、回复流等设计改动时，必须同时检查并明确是否需要联动修改普通频道消息、任务源消息卡片（`TaskRootEntry`）、任务线程回复（`TaskThreadDrawer`）、DM 消息等多种消息类型；若只改其中一种，必须在任务说明和测试中写明原因，避免样式和元数据规则分叉。
- 当用户要求按 shadcn/ui 官网或默认组件样式实现时，必须先以对应 shadcn/ui registry 源码和官方示例为基准；如果 shadcn 默认样式、组件结构或 class 与 Slei 本地封装/现有样式冲突，必须暂停并询问用户取舍，不要自行判断、改写、裁剪或用本地风格覆盖默认样式。
