...
>(({ className, ...props }, ref) => (
  <AlertDialogPortal>
    ...
    <AlertDialogPrimitive.Content
      ...
      className={cn(
        "bg-background text-foreground fixed top-[50%] left-[50%] z-50 grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border p-6 shadow-lg duration-200 sm:max-w-lg",
...
function AlertDialogMedia({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div data-slot="alert-dialog-media" className={cn("mb-2 inline-flex h-10 w-10 items-center justify-center rounded-lg border bg-muted text-muted-foreground", className)} {...props} />
}
...
type AlertDialogButtonProps<TPrimitive extends React.ElementType> =
  Omit<React.ComponentPropsWithoutRef<TPrimitive>, "className"> &
