{
  ...
  function createSleiPreloadApi() {
    return {
      rpc: {
        async call(method: string, payload: unknown) {
          return unwrapRpcInvokeResult(await ipcRenderer.invoke("slei:rpc", { method, payload }));
        },
      },
      events: {
        subscribe(channel: SleiPreloadChannel, handler: SleiPreloadHandler) {
          ...
          const subscriptionId = createDefaultSubscriptionId();
          const listener: IpcListener = (_event, payload) => handler(payload);

          ipcRenderer.on(eventName, listener);
          ipcRenderer.send("slei:events:subscribe", { channel, subscriptionId });

          return () => {
            ipcRenderer.off(eventName, listener);
            ipcRenderer.send("slei:events:unsubscribe", { subscriptionId });
          };
        },
      },
    };
