...
export function renderConnectionStep(input: {
  ...
  return `<section class="rounded-xl border border-border/60 bg-card p-4 shadow-sm" data-slei-panel data-variant="surface" data-onboarding-step="connection" data-slei-status="${escapeHtmlAttribute(status)}">${escapeHtmlText(message)}</section>`;
}
