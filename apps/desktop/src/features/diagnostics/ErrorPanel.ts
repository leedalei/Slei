...
export function renderErrorPanel(input: { locale: "zh-CN" | "en-US"; code: ErrorCode }): string {
  ...
  return `<section class="rounded-xl border border-destructive/35 bg-card p-4 text-sm text-card-foreground" data-slot="card" data-slei-diagnostics-error="${escapeHtmlAttribute(input.code)}"><strong>${escapeHtmlText(input.code)}</strong> <span>${escapeHtmlText(message)}</span></section>`;
}
