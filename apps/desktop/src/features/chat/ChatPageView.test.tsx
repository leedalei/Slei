...
describe("ChatPage mention panel", () => {
  ...
  it("renders message actions as a hover toolbar and keeps time beside the lower bubble corner", () => {
    ...
    expect(actionToolbar?.dataset.messageSide).toBe("outgoing");
    expect(actionToolbar?.className).toContain("slei-message-actions");
    ...
    expect(appCss).toContain('.slei-message-actions[data-message-side="outgoing"] {');
    expect(appCss).toContain("background: color-mix(in srgb, var(--primary) 88%, var(--background));");
  ...
  it("separates human and agent messages with outgoing and incoming bubbles", () => {
    ...
    expect(humanBubble?.className).toContain("p-3");
    expect(humanBubble?.className).not.toContain("px-3.5");
    expect(humanBubble?.querySelector<HTMLElement>(".slei-markdown-message")?.className).not.toContain("mt-1");
    ...
    expect(agentBubble?.className).toContain("p-3");
  ...
  it("keeps task root entry status and source message behavior on flat rows", () => {
    ...
    expect(taskBubble?.className).toContain("p-3");
    expect(taskBubble?.querySelector<HTMLElement>(".slei-markdown-message")?.className).not.toContain("mt-1");
  ...
  it("keeps only the composer input recessed while the outer surface stays flat", async () => {
    ...
    expect(appCss).not.toContain("inset: -12px;");
    expect(appCss).toContain("-webkit-backdrop-filter: blur(8px);");
    expect(appCss).toContain("backdrop-filter: blur(8px);");
    ...
    expect(appCss).toContain("--composer-glass-bg: rgba(255, 255, 255, 0.72);");
    ...
    expect(darkComposerCss).not.toContain("--composer-glass-bg: rgba(255, 255, 255, 0.72);");
    expect(lightComposerCss).toContain("--composer-glass-bg: rgba(255, 255, 255, 0.72);");
    ...
    const composerGlassBaseCss = appCss.slice(appCss.indexOf(".slei-composer-glass {"), appCss.indexOf(".slei-composer-glass::before {"));
    const composerGlassBeforeCss = appCss.slice(appCss.indexOf(".slei-composer-glass::before {"), appCss.indexOf(".slei-composer-glass > * {"));
    expect(composerGlassBaseCss).not.toContain("backdrop-filter:");
    expect(composerGlassBaseCss).not.toContain("-webkit-backdrop-filter:");
    expect(composerGlassBeforeCss).toContain("pointer-events: none;");
